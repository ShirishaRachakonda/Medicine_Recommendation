...
# Symptoms dictionary mapping symptom names to indexes
...
def predict():
    ...
    if invalid_symptoms:
        return render_template('index.html', message=f"Invalid symptoms entered: {', '.join(invalid_symptoms)}")

    if suggestions:
        return render_template('index.html',
                               message=f"Did you mean: {', '.join([f'{k} -> {v}' for k, v in suggestions.items()])}")
...
data = {
    ...
    "stomach pain": "Antacids (Tums, Ranitidine), Proton pump inhibitors (Omeprazole)",
